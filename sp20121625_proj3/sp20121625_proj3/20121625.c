...
int loadInstSet(){
	...
	mn_head = (INST_NODE**)calloc(20, sizeof(INST_NODE*));
	mn_tail = (INST_NODE**)calloc(20, sizeof(INST_NODE*));
	...
	while(fscanf(fp_r,"%s %s %s",opcode,mnemonic,inst_type)!=EOF){
		...
		sum = 0;
		for (i = 0; i<strlen(opcode); i++) sum += opcode[i] + (opcode[i] - 'A')*(i);
			createMnTable(createInstNode(opcode, mnemonic, inst_type), sum % 20);
	}
	return 1;
}
...
int isValidInst(int tok_cnt, char **inst_tok, int comma_cnt, char** comma_pos,int inst_no){
	...
	if((0<=inst_no && inst_no<=11)){ 
		if(tok_cnt) res = 0;
		if(comma_cnt) res = 0;
		return res;
	}
	else if (inst_no == 12) {				// bp
		if (tok_cnt>1) res = 0;
		if (comma_cnt) res = 0;
		return res;
	}
	else if(13<=inst_no && inst_no <= 16){ 	// 1 token
		if(comma_cnt) res=0;
		if(tok_cnt!=1) res =0;
		return res;
	}
	...
	if(inst_no==17 || inst_no==18){ 		// 0 to 2 tokens
		if(!tok_cnt){
			if(comma_cnt) res=0;
		}
		else if(tok_cnt==1){
	...
	else if(inst_no==19 || inst_no==20){	 // 2 tokens
		if(tok_cnt!=2) res=0;
	...
	else if(inst_no==21 || inst_no==22){ 	// 3 tokens
		if(tok_cnt!=3) res=0;
...
int main(void){
	char *sh_inst[24] = {"q","quit","h","help","d","dir","hi","history","reset","opcodelist","symbol","run","bp","opcode",
		"assemble","type","progaddr", "du","dump","e","edit","f","fill","loader"};
	...
	est_head = (ESTAB_NODE**)calloc(3, sizeof(ESTAB_NODE*));
	est_tail = (ESTAB_NODE**)calloc(3, sizeof(ESTAB_NODE*));

	if(!loadInstSet()) return 0;
	PROGADDR = 0;	//default PROGADDER
	while(1){
		tok_cnt=comma_cnt=0;
		inst_flag=op_head[0];
		input = buf;
		...
		for(i=0;i<24;i++) if(!strcmp(input,sh_inst[i])) break; 								// check if is valid shell command
		
		if(!isValidInst(tok_cnt,inst_tok,comma_cnt,comma_pos,i)||23<i||(MAX_INST_LEN<len)){ // check if is valid input format
		...
		case 11:run();
				break;
		case 12:if (!breakPoint(inst_tok[0])) break;
				break;
		case 13:inst_flag=opcodeMnemonic(inst_tok[0],'s');
				break;
		case 14:if((inst_tok[1]=strchr(inst_tok[0],'.'))) 
				...
				break;
		case 15:type(inst_tok[0]);
				break;
		case 16:if ((PROGADDR = hexToDec(inst_tok[0], 'a')) == -1) inst_flag = NULL;
				else if (PROGADDR > MAX_MEM) inst_flag = NULL;
				else break;
		case 17:
		case 18:dump(tok_cnt,inst_tok[0],inst_tok[1]);
				break;
		case 19:
		case 20:v_mem[hexToDec(inst_tok[0],'s')]=hexToDec(inst_tok[1],'s');
				break;
		case 21:
		case 22:fill(inst_tok[0], inst_tok[1], inst_tok[2]);
				break;
		case 23:freeESTAB();
				loader(tok_cnt,inst_tok);
				break;
		}
		if(!inst_flag){
			printf("invalid instruction\n");
